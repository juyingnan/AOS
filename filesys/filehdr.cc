...
bool FileHeader::Append(int bytesToAdd) {
	...
	for(int i = 1; i <= sectorsToAdd; i++) {
		...
		if(vAddr + MaxDirectSize < NumDirect) {//Direct index
			...
			if(L1Index < 0) {
				L1Index = currentFreeMap->Find();
			} else {
				sinode->FetchFrom(L1Index);
			}
...
FileHeader::ByteToSector(int offset)
{
	...
	if(virtualAddr < MaxDirectSize) {//direct
		...
		ASSERT(L3Index >= 0 && L2Index >= 0 && L1Index >= 0);
		tinode->FetchFrom(L3Index);
...
int FileHeader::ByteToINodeSector(int offset) {
	...
	if(virtualAddr < MaxDirectSize) {//direct
		...
		if(virtualAddr < MaxDirectSize + MaxSingleSize) {//Single index //Assume that there is enough space
			virtualAddr -= MaxDirectSize;
			//			printf("Single index is %d, singleSize is %d,vAddr is %d*********\n",L1Index,MaxDirectSize + MaxSingleSize,virtualAddr);
			if(L1Index < 0) {//no single index yet
				//				printf("Single index not exist\n");
				L1Index = ((BitMap *)currentFreeMap)->Find();//allocate a sector for single inode
			} else {
				//				printf("L1Index already exist,fetch from %d\n",L1Index);
				sinode->FetchFrom(L1Index);
			}
			...
			//			printf("hey im here\n");
			//			if(!checkS) {
			sinode->WriteBack(L1Index);//write single inode back to disk
			//			}

		} else if(virtualAddr < MaxDirectSize + MaxSingleSize + MaxDoubleSize) {//Double index
			virtualAddr -= MaxDirectSize + MaxSingleSize;
			//			printf("Double index\n");
			if(L2Index < 0) {//no double index yet
				L2Index = ((BitMap *)currentFreeMap)->Find();//allocate a sector for double inode
			} else {
				dinode->FetchFrom(L2Index);
			}
			...
			//			if(!checkS || !checkD) {
			dinode->WriteBack(L2Index);//write double inode back to disk
			sinode->WriteBack(sSector);
			//			}
		} else if(virtualAddr < MaxDirectSize + MaxSingleSize + MaxDoubleSize + MaxTripleSize) {//Triple index
			virtualAddr -= MaxDirectSize + MaxSingleSize + MaxDoubleSize;
			if(L3Index < 0) {//no triple index yet
				L3Index = ((BitMap *)currentFreeMap)->Find();//allocate a sector for triple inode
			} else {
				tinode->FetchFrom(L3Index);
			}
			//			printf("Triple index\n");
